...
from sqlxport.cli.shared_validations import validate_options, warn_if_s3_endpoint_suspicious


@click.command("run-export")
...
@click.option('--s3-provider', default="aws", type=click.Choice(["aws", "minio"], case_sensitive=False),
              help='S3 provider (aws or minio)')
@click.option('--verbose', is_flag=True, help='Print debug output')
def run_export_command(db_url, query, output_file, output_dir, partition_by,
                       format, export_mode, iam_role, s3_output_prefix,
                       s3_bucket, s3_key, s3_endpoint, s3_access_key, s3_secret_key,
                       aws_region, upload_output_dir, env_file, s3_provider, verbose):
    """Run SQL query and export results to Parquet/CSV"""
    if env_file:
        print(f"🔍 Loading environment overrides from {env_file}")
        load_dotenv(dotenv_path=env_file)

    validate_options(db_url, query, output_file, output_dir, partition_by)
