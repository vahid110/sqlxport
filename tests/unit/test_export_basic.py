...
def test_export_upload_file_to_s3(tmp_path, monkeypatch):
    uploaded = {}
    monkeypatch.setattr("sqlxport.api.export.upload_file_to_s3", lambda file_path, bucket_name, object_key: uploaded.update({
        "file_path": file_path, "bucket": bucket_name, "key": object_key
    }))
